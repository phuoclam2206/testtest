...
var clientClassKoreanController = {
    ...
    fetchDetail: function (req, res, next) {
        ClassKoreanPost.findOne({"_id": req.params.id}, function (err, result) {
            if(err) next(err);
    ...
    fetchMostView: function (req, res, next) {
        var query = ClassKoreanPost.find({}, {title: 1, image: 1, _id: 1, created_date: 1}).sort({"view": -1}).limit(4);
...
module.exports = clientClassKoreanController;
