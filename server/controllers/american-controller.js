...
var americanController = {
    create: function (req, res) {
        image.resize(req.file, function (err, newPath) {
            if(err) next(err);
            var americanStudyAboardPost = new AmericanStudyAboardPost({
            ...
            americanStudyAboardPost.save();
            res.json(americanStudyAboardPost);
        });
    },
    ...
    delete: function (req, res) {
        AmericanStudyAboardPost.remove({_id: req.params.id}).exec(function (err) {
            if (err) next(err);
...
module.exports = americanController;
